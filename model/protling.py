...
def evaluate_and_log(log_file, configs, models, session):
    ...
    if configs['training'].curriculum['mode'] is not None:
        ...
        extended_log = ('\tCurriculum Iteration: {curriculum_step:.3f}\t'
                        ...
                        + unwt_val_loss_subgroups_string).format(
            unwt_train_loss,
            unwt_val_loss['tertiary_loss_all'],
            unwt_test_loss,
            **merge_dicts(diagnostics, unwt_val_loss)
        )
    else:
        extended_log = ''
...
def loop(args_):
    ...
    configs.update({'training': RGNConfig(args_.config_file,
                                          {'name': 'training',
                                           ...
                                           'fillGPU': args_.fill_gpu})})

    configs.update({'evaluation': RGNConfig(args_.config_file,
    ...
    if args_.evaluation_model:
        ...
        for entry in args_.evaluation_model:
    ...
    if args_.prediction_only:
        ...
        try:
            ...
            if args_.restart_on_missed_milestone:
                ...
                new_seed = old_seed + args_.seed_increment
                for line in fileinput.input(args_.config_file, inplace=True):
...
if __name__ == '__main__':
    ...
    parser.add_argument('-m',
                        '--milestone',
                        type=lambda m: map(float, m.split(':')),
                        action='append',
                        help='milestone that the model must achieve or it will be restarted. '
                             + 'milestones must be of the form step:loss. multiple milestones can be set.')

    parser.add_argument('-r',
                        '--restart_on_dead_gradient',
                        action='store_true',
    ...
    gpu_group.add_argument('-a',
                           '--fill_gpu',
                           action='store_true',
                           help='fill all available GPU memory')

    gpu_group.add_argument('-f',
                           '--gpu_fraction',
                           type=float,
                           help='fill only specified GPU memory fraction')
