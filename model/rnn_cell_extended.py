...
class ZoneoutWrapper(RNNCell):
    ...
    """
        ...
        super(ZoneoutWrapper, self).__init__(**kwargs)
        if isinstance(memory_cell_keep_prob, float) and not (0.0 <= memory_cell_keep_prob <= 1.0):
            raise ValueError("Parameter memory_cell_keep_prob must be between 0 and 1: %d"
                             % memory_cell_keep_prob)
        if isinstance(hidden_state_keep_prob, float) and not (0.0 <= hidden_state_keep_prob <= 1.0):
    ...
    def __call__(self, inputs, state, scope=None):
        ...
        if self._has_hidden_state_zoneout or self._has_memory_cell_zoneout:
            ...
            c_and_m = []
            for s_old, s_new, p, has_zoneout\
                    in [(c_old, c_new, self._memory_cell_keep_prob, self._has_memory_cell_zoneout),
                        (m_old, m_new, self._hidden_state_keep_prob, self._has_hidden_state_zoneout)]:
                if has_zoneout:
                    if self._is_training:
                        mask = nn_ops.dropout(array_ops.ones_like(s_new),
                                              p,
                                              seed=self._seed)\
