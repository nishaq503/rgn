    ...
    def _create_graph(self, mode, config):
        ...
        with tf.variable_scope(SCOPE, reuse=(mode == 'evaluation')) as scope, \
            ...
            if config.loss['tertiary_weight'] > 0:
                ...
                dihedrals_config = merge_dicts(config.initialization, config.optimization, config.architecture,
                ...
                coordinates = _coordinates(merge_dicts(config.computing, config.optimization, config.queueing),
                                           dihedrals)
                drmsds = _drmsds(merge_dicts(config.optimization, config.loss, config.io), coordinates, tertiaries,
                                 weights)
            ...
            if config.loss['include']:
                filters = {grp: id_filter(ids, grp) for grp in
                           config.io['evaluation_sub_groups']} if mode == 'evaluation' else {}
            ...
            if mode == 'training' and config.curriculum['behavior'] in ['fixed_rate', 'loss_threshold', 'loss_change']:
                curriculum_update_op = _curriculum(config.curriculum, curriculum_step, curriculum_loss_history,
                                                   [minimize_op])
        ...
        with tf.variable_scope('sublayer' + str(layer_idx) + (name if name is '' else '_' + name),
            ...
            for case in switch(config['recurrent_unit']):
                if case('Basic'):
                    ...
                    cell = tf.contrib.rnn.LayerNormBasicLSTMCell(num_units=layer_size,
                                                                 forget_bias=config['recurrent_forget_bias'],
                                                                 layer_norm=config['recurrent_layer_normalization'],
                                                                 dropout_keep_prob=keep_prob, reuse=(not is_training))
                elif case('LSTMBlock'):
                    cell = tf.contrib.rnn.LSTMBlockCell(num_units=layer_size,
                                                        forget_bias=config['recurrent_forget_bias'],
...
def _dihedrals(mode, config, inputs, alphabet=None):
    ...
    if config['is_alphabetized']:
        ...
        probs = tf.nn.softmax(flattened_linear / config['alphabet_temperature'],
        ...
        dihedrals = tf.reshape(flattened_dihedrals,
                               [num_steps, batch_size, NUM_DIHEDRALS])  # [NUM_STEPS, BATCH_SIZE, NUM_DIHEDRALS]
    else:
        # just linear
        dihedrals = linear
...
def _accumulate_loss(config, numerator, denominator, name_prefix=''):
    ...
    if config['num_evaluation_invocations'] == 1:
        ...
        with tf.control_dependencies([update_op]):
            accumulated_loss = tf.divide(numerator_accumulator, denominator_accumulator,
