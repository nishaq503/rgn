...
class Config(object):
    ...
    def __init__(self, infile=None, config=None):
        ...
        # load configuration values from infile if available
        if config is None:
            config = {}
        if infile is not None:
            file_config = dict_import(infile)
...
class RGNConfig(Config):
    ...
    def _create_config(self, config):
        ...
        self.io = {'name': config.get('name', None),
                   ...
                   'checkpoint_every_n_hours': int(
                       config.get('checkpointEveryNHours', 24))}  # this is in addition to the max_checkpoints
        ...
        self.computing = {'num_cpus': int(config.get('numCPUs', 4)),
                          ...
                          'functions_on_devices': eval_if_str(
                              config.get('functionsOnDevices', {'/cpu:0': ['point_to_coordinate']})),
        ...
        self.initialization = {'graph_seed': int_or_none(config.get('randSeed', None)),
                               ...
                               'recurrent_out_proj_init': eval_if_str(
                                   config.get('recurrentOutProjInit', {'base': {}, 'bias': {}})),
                               'recurrent_out_proj_seed': int_or_none(config.get('recurrentOutProjSeed', None)),
                               'recurrent_nonlinear_out_proj_init': eval_if_str(
                                   config.get('recurrentNonlinearOutProjInit', {'base': {}, 'bias': {}})),
                               'recurrent_nonlinear_out_proj_seed': int_or_none(
                                   config.get('recurrentNonlinearOutProjSeed', None)),
        ...
        self.optimization = {'optimizer': config.get('optimiser', 'steepest'),
                             ...
                             # momentum, rmsprop, has no analog in autograd
                             'beta1': float(config.get('beta1', 0.9)),  # adam, momentum in autograd
                             'beta2': float(config.get('beta2', 0.999)),  # adam, hoMomentum in autograd
                             'epsilon': float(config.get('epsilon', 10e-8)),
                             # adam, rmsprop, adadelta. this should really be 1e-8
                             ...
                             'recurrent_threshold': flt_or_none(config.get('recurrentThreshold', None)),
                             # only TF-based RNNs
                             ...
                             # Longer seqs removed, shorter ones padded. Max irrespective of curriculum
                             'num_epochs': int_or_none(config.get('numEpochs', None))}

        # queueing
        self.queueing = {'file_queue_capacity': int(config.get('fileQueueCapacity', 1000)),
                         # Defaults make sense if each file has ~100 sequences
        ...
        self.architecture = {'recurrent_unit': config.get('recurrentUnit', 'LSTM'),
                             ...
                             'include_recurrent_outputs_between_layers': str_or_bool(
                                 config.get('includeRecurrentOutputsBetweenLayers', True)),  # HO
                             'include_dihedrals_between_layers': str_or_bool(
                                 config.get('includeDihedralsBetweenLayers', False)),  # HO
                             'residual_connections_every_n_layers': int_or_none(
                                 config.get('residualConnectionsEveryNLayers', None)),  # HO
                             'first_residual_connection_from_nth_layer': int_or_none(
                                 config.get('firstResidualConnectionFromNthLayer', 1)),  # HO
                             'recurrent_to_output_skip_connections': str_or_bool(
                                 config.get('recurrentToOutputSkipConnections', False)),  # HO
                             'input_to_recurrent_skip_connections': str_or_bool(
                                 config.get('inputToRecurrentSkipConnections', False)),  # HO
                             'all_to_recurrent_skip_connections': str_or_bool(
                                 config.get('allToRecurrentSkipConnections', False)),  # HO
                             'recurrent_nonlinear_out_proj_size': eval_if_str(
                                 config.get('recurrentNonlinearOutputProjSize', None)),
                             'recurrent_nonlinear_out_proj_function': config.get('recurrentNonlinearOutputProjFunction',
                                                                                 'tanh'),
        ...
        self.regularization = {'recurrent_input_keep_probability': eval_if_str(config.get('recurInKeepProb', 1.0)),
                               ...
                               'recurrent_state_zonein_probability': eval_if_str(
                                   config.get('recurStateZoneinProb', 1.0)),
                               'recurrent_memory_zonein_probability': eval_if_str(
                                   config.get('recurMemoryZoneinProb', 1.0)),
                               ...
                               'recurrent_nonlinear_out_proj_normalization': str_or_none(
                                   config.get('recurNonlinearOutProjNormalization', None)),
                               'recurrent_layer_normalization': str_or_bool(
                                   config.get('recurLayerNormalization', False)),  # LNLSTM
                               'recurrent_variational_dropout': str_or_bool(
                                   config.get('recurVariationalDropout', False))}
...
class RunConfig(Config):
    ...
    def _create_config(self, config):
        ...
        self.optimization = {'validation_milestone': eval_if_str(config.get('validationMilestone', {})),
                             'validation_reference': config.get('validationReference',
                                                                'weighted')}
        # '[un]weighted', used for milestones, curricula, and predictions
        ...
        self.evaluation = {'num_training_samples': int(config.get('numTrainingSamples', 98)),
                           ...
                           'num_training_invocations': int(config.get('numTrainingInvocations', 1)),
                           # evaluation (! actual training)
                           ...
                           'include_unweighted_validation': str_or_bool(
                               config.get('includeUnweightedValidation', False)),
        ...
        self.loss = {'training_tertiary_normalization': config.get('trainingTertiaryNormalization', 'first'),
                     ...
                     'evaluation_batch_dependent_normalization': config.get('evaluationBatchDependentNormalization',
                                                                            True)}
